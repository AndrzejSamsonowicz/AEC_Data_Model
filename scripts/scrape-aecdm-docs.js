/**
 * AEC Data Model API Documentation Scraper
 * Fetches all pages via r.jina.ai (handles JS-rendered SPAs) and writes
 * a single consolidated markdown file. Supports --resume to skip already-
 * downloaded sections without re-fetching.
 *
 * Usage:
 *   node scripts/scrape-aecdm-docs.js           # fresh run
 *   node scripts/scrape-aecdm-docs.js --resume  # continue after a rate-limit interruption
 *
 ...
 *
 * Note: jina.ai free tier ~200 req/day. This doc has ~170 pages.
 *       If rate-limited, re-run with --resume the next day.
 */
...
const DELAY_MS = 900;   // polite delay between requests
...
const PAGES = [
  ...
  { section: 'How-to Guide', title: 'Task 6 – Add, Update and Remove Properties Using Category Filter', url: `${BASE}/tutorials/extend_element_data/category_binding_filter_workflow` },
...
/** Fetch one page via r.jina.ai and return the raw markdown */
...
/** Build the section heading block for a page (written before content) */
...
/** Returns a Set of titles already present in the output file */
...
async function main() {
  ...
  if (RESUME) {
    console.log(`Resuming — ${doneTitles.size} pages already done, ${toFetch.length} remaining.\n`);
  } else {
    // Fresh run: write file header
    const header = [
      '# AEC Data Model API — Full Documentation\n',
      `> Auto-generated on ${new Date().toISOString().slice(0, 10)} by scripts/scrape-aecdm-docs.js\n`,
      `> Source: ${BASE}/developers_guide/overview\n`,
      '---\n',
    ].join('\n');
    fs.writeFileSync(OUTPUT_FILE, header, 'utf8');
    console.log(`Fresh run — ${PAGES.length} pages to fetch → ${OUTPUT_FILE}\n`);
  }
  ...
  for (let i = 0; i < toFetch.length; i++) {
    ...
    try {
      ...
      content = `_(Failed to fetch: ${err.message})_`;
      failCount++;
      console.log(`FAILED — ${err.message}`);

      // On 429 rate-limit: stop immediately so the partial file is saved
      if (err.response && err.response.status === 429) {
        fs.appendFileSync(OUTPUT_FILE, heading + content + '\n', 'utf8');
        console.log('\n⚠  Rate limited (429). Partial progress saved.');
        console.log('   Re-run with --resume tomorrow:\n   node scripts/scrape-aecdm-docs.js --resume');
        process.exit(1);
      }
    }

    // Write this page immediately (incremental — survives interruption)
    fs.appendFileSync(OUTPUT_FILE, heading + content + '\n', 'utf8');

    if (i < toFetch.length - 1) await delay(DELAY_MS);
  }

  const sizekb = (fs.statSync(OUTPUT_FILE).size / 1024).toFixed(1);
  console.log(`\nDone. ${successCount} succeeded, ${failCount} failed.`);
  console.log(`Output: ${OUTPUT_FILE}  (${sizekb} KB)`);
  console.log(`\nNext step: node scripts/clean-docs.js --file "API Documentation/AEC-DataModel-API.md"`);
}
